...
# Determine absolute PATH of OpenShot repo
...
log.info("Execution path: %s" % PATH)
...
if os.path.exists(qt_system_path):
    for file in os.listdir(qt_system_path):
        # Add QT translation files to include list
        if (file.startswith("qt_") or file.startswith("qtbase_")) and file.endswith(".qm"):
            src_files.append((os.path.join(qt_system_path, file), "language/{}".format(file)))
...
if sys.platform == "win32":
    ...
    # if not os.path.exists(libopenshot_path):
    #     # Try the archlib path
    #     libopenshot_path = ARCHLIB
    # if not os.path.exists(libopenshot_path):
...
try:
    ...
    log.error("Failed to create frozen executable", exc_info=1)
